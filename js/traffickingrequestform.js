...
function generateOutput() {
  ...
  let mobLp1 = getValueById('mobLp1');
  if (mobLp1) {
    let mobLp1Lc = mobLp1.toLowerCase();
    if (!regex.test(mobLp1)) {
      $('#modal-trigger')[0].click();
      getElementById('modal1text').innerHTML = "Please enter a proper URL."
      return;
    }
...
`;
  ...
  adDimensionsSelected.forEach(function(subAdDimensionsSelected, indexSubAdDimnsionSelected) {
    platforms.forEach(function(platformSelected, indexPlatforms) {
      ...
      tsData.push(brand, country, truncatedPlatform, campaignName, budgetCode, agency, buyingPlatforms, publisherOrNetwork.trim(), subSite.trim(), audience, vertical.trim(), message.trim(), offer.trim(), subAdDimensionsSelected.trim(), targeting, subTargeting, deliverables);
      ...
      tsDtFrPlacmntNme.push(brandCode, country, truncatedPlatform, campaignName, budgetCode, agency, buyingPlatforms, publisherOrNetwork.trim(), subSite.trim(), audience, vertical.trim(), message.trim(), offer.trim(), amsId, subAdDimensionsSelected.trim(), targeting, subTargeting);
...
function fnExcelReport() {
  ...
  handleSignInClick();
  updateSignInStatus();
  batchUpdateValues('1of8Qdd8SYiYOlpjztSlLy09YqSIsg0or0iqD5uXq9Vc', 'Sheet2!A:C', 'RAW', updatingValues, callback);
}
...
let updatingValues = [amsIdUsedArr, placementNameUsedArr];
...
function getValues(spreadsheetId, range, callback) {
  ...
  gapi.client.sheets.spreadsheets.values.get({
    ...
    var collectAmsData = Array.from(result.values);
    console.log("collectAmsData.length   " + collectAmsData.length);
    for (i = 1; i < collectAmsData.length; i++) {
      if (collectAmsData[i].toString().length <= 30) {
        let amsId = collectAmsData[i].toString().split(",");
        console.log("AMSID  " + amsId[1]);
        amsIdArr.push(amsId[1]);
      }
    }
    // [START_EXCLUDE silent]
    callback(response);
